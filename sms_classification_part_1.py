...
labels = OrderedDict([
        ...
        (" __time__ ", '(2[0-3]|1[0-9]|0[0-9]|[0-9]):([0-5][0-9])'),
...
def tokenize(sentence, is_lower=True, is_strip=True, stopwords=None,
    ...
    # for token in tokenized_sent:
        ...
        if (" %s "%token not in labels.keys()) and (not eng_vocab.check(token)) and (lemma != 'entity'):
            continue
        ...
        if (prev_feature == lemma):
            continue
        #chk for removing single and double letter words to reduce noise
...
def train_svc(train_features, correct_labels):
...
def test_svc(testing_features, svc=None):
    # vectorizer = TfidfVectorizer("english")
    if not svc:
        svc = load_model("SVC")
...
def main():
    ...
    comb_feat = pd.concat([train_features, test_features])
    train_features = vectorizer.fit_transform(comb_feat)
    limit = 30000
    train_features = train_features[:limit]
    test_features = train_features[limit:]
